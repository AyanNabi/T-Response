...
class _ImageUploadButtonState extends State<ImageUploadButton> {
  ...
  Future<void> _pickImage() async {
    ...
    if (image != null) {
      setState(() {
        if (kIsWeb) {
          // For web, use the image URL directly
          _image = null; // Ensure _image is null for web
          widget.onImageSelected(null); // Pass image URL back
        } else {
          // For mobile, use the File
          _image = File(image.path);
          widget.onImageSelected(_image); // Pass image back
        }
      });
    }
        ...
        child: Center(
          child: _image == null
            ...
            style: GoogleFonts.poppins(
              ...
              : Image.file(_image!, fit: BoxFit.cover),  // Use Image.file for other platforms
        ),
        ),
